...
The final implementation applies a temporary, one-shot boost to tasks waking after a long uninterruptible sleep. In the synthetic benchmark, the IQR-filtered scenario means show a reduction in target-task runnable wait time between 75.2\% and 96.9\% across the tested contention levels.
\end{abstract}
...
\begin{itemize}
    \item \textbf{Host:} Ubuntu 24.04.4 on an Intel Core i7-6700, with the QEMU process restricted to isolated logical CPUs (\texttt{taskset -c 2,3}) and assigned real-time priority (\texttt{chrt -r 99}) to reduce host-side interference. These logical CPUs belong to separate physical cores, although their SMT siblings 6 and 7 were not isolated.
...
To solve this, working on a PC running Linux natively (rather than in a virtual machine) is essential because GRUB can be configured to isolate one or more logical CPUs at boot time
and reduce ordinary host scheduling on them. In this setup, the isolated logical CPUs were used through the QEMU configuration as follows:

\begin{itemize}
    \item Modify this line in /etc/default/grub to isolate logical CPUs 2 and 3: \\
    ...
    \item Restrict the complete QEMU process to the host CPU set composed of logical CPUs 2 and 3. This does not enforce a permanent one-to-one mapping between a host CPU and a guest vCPU.
...
The complete benchmark of the final patch was run on August 25, 2026. It measured 100 iterations at each load from 1 to 12 competing CPU-bound processes for both the vanilla and patched kernels, giving 1,200 samples per kernel and 2,400 samples in total. The complete invocation took just over 14 hours. The benchmark first measured the unmodified fair scheduler, then applied the patch and repeated the same workload.

Table~\ref{tab:final-results} reports the means after applying the IQR filter independently to each kernel and load scenario. Every tested scenario shows lower runnable wait time with the patch. The smallest reduction is 75.2\% with 4 competing processes, while the largest is 96.9\% with 1 competing process. The equal-weight reduction across the 12 filtered scenario means is 79.5\%.
...
\begin{tabular}{rrrr}
\toprule
CPU children & Vanilla (ms) & Patched (ms) & Reduction \\
\midrule
1  & 1948.47  & 60.88   & 96.9\% \\
2  & 2104.01  & 420.78  & 80.0\% \\
3  & 3455.38  & 734.10  & 78.8\% \\
4  & 4632.36  & 1150.53 & 75.2\% \\
5  & 5834.82  & 1356.91 & 76.7\% \\
6  & 6876.73  & 1594.71 & 76.8\% \\
7  & 7727.35  & 1708.93 & 77.9\% \\
8  & 8657.75  & 1800.17 & 79.2\% \\
9  & 9324.75  & 2041.04 & 78.1\% \\
10 & 9862.80  & 1810.89 & 81.6\% \\
11 & 10399.17 & 1985.55 & 80.9\% \\
12 & 10804.12 & 2088.70 & 80.7\% \\
\bottomrule
\end{tabular}
\caption{IQR-filtered mean \texttt{wait\_sum} for 100 iterations per scenario.}
\label{tab:final-results}
\end{table}

Figure~\ref{fig:final-results} shows the same means together with their standard deviations. The raw, unfiltered data produces the same conclusion: its equal-weight mean reduction is also 79.5\%, and all 12 scenarios improve. IQR filtering removed 21 of 1,200 vanilla samples and 30 of 1,200 patched samples.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{benchmark_comparison.png}
    \caption{Target-task runnable wait time before and after the wakeup boost. Error bars show the standard deviation after per-scenario IQR filtering.}
    \label{fig:final-results}
\end{figure}

The result supports a performance claim only for this controlled workload: the patch reduces the measured runnable wait time of the target task under the tested contention. Both kernels completed all scenarios and shut down normally. The identical Vexpress boot and device warnings in both logs are unrelated to the scheduler modification.
...
There are still limits which are important to keep clear: the workload is synthetic, the unmodified run is completed before the patched run rather than interleaving their samples, and the experiment does not measure CPU throughput, starvation or wider system stability. The completed benchmark establishes the measured reduction for this controlled workload, but it does not establish a general performance improvement for real I/O workloads.
